...
export default function DetailPage() {
  ...
  const themeColor = mode === 'cafe' ? 'blue' : 'orange';
  const themeBg = mode === 'cafe' ? 'bg-blue-600' : 'bg-orange-500';
  const themeText = mode === 'cafe' ? 'text-blue-600' : 'text-orange-500';
  const themeBorder = mode === 'cafe' ? 'border-blue-600' : 'border-orange-500';
  ...
  useEffect(() => {
    ...
    const endpoint = mode === 'cafe' ? `/api/cafes/${id}` : `/api/kuliners/${id}`;
    
    fetch(`${process.env.NEXT_PUBLIC_API_URL}${endpoint}`)
      .then(res => res.json())
      .then(data => {
        if (data.error) router.push('/explore?mode=' + mode);
        else setPlace(data);
      })
      .catch(() => router.push('/explore?mode=' + mode));

    fetchReviews(endpoint);
  }, [mode, id, router]);

  const fetchReviews = (endpoint: string) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}${endpoint}/reviews`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = mode === 'cafe' ? `/api/cafes/${id}/reviews` : `/api/kuliners/${id}/reviews`;
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: currentUser.id, rating: newRating, comment: newComment, type: mode })
      });
      if (res.ok) {
        setNewComment('');
        setNewRating(5);
        fetchReviews(mode === 'cafe' ? `/api/cafes/${id}` : `/api/kuliners/${id}`); 
  ...
  const rawGallery = place ? safeParse(place.gallery, []) : [];
  const allPhotos = place ? [place.imageUrl, ...rawGallery].filter(Boolean) : [];
  ...
  const checkStatus = (data: any) => {
    if (data.is24Hours) return { isOpenNow: true, statusText: '🟢 Buka Sekarang' };
    let opHours = data.operationalHours;
  ...
  const crowdBadge = getCrowdBadge(place.crowdStatus);

  const parsedFacs = safeParse(place.facilities, []);
  const placeAreas = parsedFacs.filter((f: string) => areaOptions.includes(f));
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <>
                <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-3 pb-2 pt-2 scrollbar-hide">
                  {allPhotos.map((photo: string, idx: number) => (
                    <div key={idx} className="w-[85%] shrink-0 aspect-[4/3] rounded-2xl overflow-hidden cursor-pointer relative snap-center shadow-md" onClick={() => setCurrentImageIndex(idx)}>
                      <img src={photo} alt={`Suasana ${idx + 1}`} className="w-full h-full object-cover active:scale-95 transition-transform" />
                      <div className="absolute bottom-3 right-3 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-white text-[10px] font-bold tracking-widest">{idx + 1} / {allPhotos.length}</div>
                    </div>
                  ))}
                </div>
                ...
                {allPhotos.length > 1 && (
                  <div className="flex md:hidden justify-center gap-1.5 mt-3 mb-2 z-20 pointer-events-none">
                    {allPhotos.map((_, idx) => (
                      <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${currentImageIndex === idx ? `w-4 ${themeBg} shadow-sm` : 'w-1.5 bg-blue-100'}`}></div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-3">
                {menusList.length > 0 ? menusList.map((menu: any, idx: number) => (
                  <div key={idx} className={`flex justify-between items-center p-4 rounded-xl border ${mode === 'cafe' ? 'bg-blue-50 border-blue-100' : 'bg-orange-50 border-orange-100'}`}>
                    ...
                    <span className={`font-black text-sm ${themeText}`}>Rp {menu.price?.toLocaleString('id-ID')}</span>
                  </div>
                )) : (
          ...
          {(delivery.gojek || delivery.grab || delivery.shopeeFood) && (
            <section className="bg-white p-6 md:hidden rounded-3xl shadow-xl border border-gray-100">
              ...
              <div className="flex flex-col gap-3">
                {delivery.gojek && <a href={delivery.gojek} target="_blank" rel="noreferrer" className="w-full bg-green-500 hover:bg-green-600 text-white font-black py-4 rounded-2xl text-center text-xs uppercase tracking-widest shadow-md">Pesan di GoFood</a>}
                {delivery.grab && <a href={delivery.grab} target="_blank" rel="noreferrer" className="w-full bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-2xl text-center text-xs uppercase tracking-widest shadow-md">Pesan di GrabFood</a>}
                {delivery.shopeeFood && <a href={delivery.shopeeFood} target="_blank" rel="noreferrer" className="w-full bg-orange-500 hover:bg-orange-600 text-white font-black py-4 rounded-2xl text-center text-xs uppercase tracking-widest shadow-md">Pesan di ShopeeFood</a>}
              </div>
            </section>
          )}
                ...
                <div key={rev.id} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-black shadow-md ${mode === 'cafe' ? 'bg-gradient-to-tr from-blue-500 to-purple-500' : 'bg-gradient-to-tr from-orange-500 to-red-500'}`}>
            ...
            {place.is24Hours ? (
              ...
              <div className="space-y-2">
                {daysOrder.map(day => {
                  ...
                  let hoursObj = place.operationalHours;
                  if (typeof hoursObj === 'string') { try { hoursObj = JSON.parse(hoursObj); } catch(e) { hoursObj = {}; } }
                  const hours = hoursObj[day];

                  return (
                    <div key={day} className={`flex justify-between items-center p-3 rounded-xl transition-all ${isToday ? `${themeBg} text-white shadow-md scale-105 my-3` : 'bg-gray-50 text-gray-600'}`}>
                      <span className={`font-black text-sm uppercase tracking-widest ${isToday ? 'text-white' : 'text-gray-900'}`}>
                        {day} {isToday && <span className={`text-[9px] bg-white ${themeText} px-2 py-0.5 rounded ml-2`}>HARI INI</span>}
                      </span>
